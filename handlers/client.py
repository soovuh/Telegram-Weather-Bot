...
async def see_weather_now(callback: types.CallbackQuery):
    ...
    check_user = await postgres_db.check_user(callback.from_user.id)
    if check_user:
        city = await postgres_db.sql_get_city(callback.from_user.id)
        cords = await postgres_db.sql_get_coords(callback.from_user.id)
        lat, lon = cords
...
async def morning_alert(callback: types.CallbackQuery):
    check = await postgres_db.add_alert_time(callback.from_user.id, 'morning')
    if check:
...
async def noon_alert(callback: types.CallbackQuery):
    check = await postgres_db.add_alert_time(callback.from_user.id, 'noon')
    if check:
        await callback.answer('Готово!')
        await bot.send_message(callback.from_user.id, 'Чи хочете ви отримувати анекдот разом з погодою', reply_markup=joke_kb)
    else:
...
async def evening_alert(callback: types.CallbackQuery):
    check = await postgres_db.add_alert_time(callback.from_user.id, 'evening')
    if check:
        await bot.send_message(callback.from_user.id, 'Чи хочете ви отримувати анекдот разом з погодою', reply_markup=joke_kb)
        await callback.answer('Готово!')
    else:
...
async def cancel_alert(callback: types.CallbackQuery):
    check = await postgres_db.delete_alert(callback.from_user.id)
    if check:
...
async def send_alert(alert_time):
    users_ids = await postgres_db.get_users_to_alert(alert_time)
    if users_ids == False:
        return
    for user_id in users_ids:
        check_user = await postgres_db.check_user(user_id)
        if check_user:
            city = await postgres_db.sql_get_city(user_id)
            cords = await postgres_db.sql_get_coords(user_id)
            lat, lon = cords
...
async def send_joke_true(callback: types.CallbackQuery):
    await postgres_db.send_joke_status(callback.from_user.id, True)
    await bot.send_message(callback.from_user.id, 'Зміни збережені!', reply_markup=client_kb)
    await callback.answer('Добре!')


async def send_joke_false(callback: types.CallbackQuery):
    await postgres_db.send_joke_status(callback.from_user.id, False)
    await bot.send_message(callback.from_user.id, 'Зміни збережені!', reply_markup=client_kb)
    await callback.answer('Добре!')
...
async def send_jokes(alert_time):
    users_ids = await postgres_db.get_users_to_send_joke(alert_time)
    if users_ids == False:
        return
    for user_id in users_ids:
        check_user = await postgres_db.check_user(user_id)
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        choose_alert_time, text='/set_alert_time')
    ...
    dp.register_callback_query_handler(
        send_joke_false, text='/send_joke_false')
